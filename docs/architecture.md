...
8. `main.py` y `src/dashboard/app.py`: puntos de entrada para uso final. El dashboard usa `@st.cache_resource` para el grafo y `@st.cache_data` para resultados del pipeline, evitando recomputaciones innecesarias.
...
- **Matriz de Costos** (`src/experimental/matrix.py`): Genera eficientemente matrices de costo y tiempo. `generate_both()` calcula ambas matrices en una sola pasada de Dijkstra por nodo usando `dijkstra_two_weights()`, reduciendo el tiempo de cálculo a la mitad.
